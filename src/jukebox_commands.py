...
class Commands(commands.Cog, name=config.COG_COMMANDS):
    ...
    async def send_versions(self, ctx: Context) -> None:
        ...
        packages: dict = {d.name: d for d in metadata.distributions()}
        for key in config.PACKAGE_CHECKS:
            ...
            msg = strings.get("info_package_major_entry").format(
                meta_dict["Version"],
                meta_dict["Summary"],
                meta_dict["Project-URL"])
            semver_current: Version = Version(meta_dict["Version"])
            for release_key in reversed(releases.keys()):
                # Append update notice for newer versions of major packages
                semver_release: Version = Version(release_key)
                # Ignore unstable releases
                ignore: bool = semver_release.is_prerelease or semver_release.is_devrelease
                if semver_release > semver_current:
                    if not ignore:
