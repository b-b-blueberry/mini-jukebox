...
async def is_trusted(ctx: Context, send_message: bool = True) -> bool:
    facts = _check_roles(ctx.author, [config.ROLE_TRUSTED, config.ROLE_JUKEBOX]) or await is_admin(ctx, send_message=False)
    if not facts and send_message:
        msg = strings.get("error_command_role_permissions")
        await ctx.reply(content=msg)
    return facts
...
async def is_default(ctx: Context, send_message: bool = True) -> bool:
    facts = _check_roles(ctx.author, [config.ROLE_DEFAULT]) or await is_trusted(ctx, send_message=False)
    if not facts and send_message:
        msg = strings.get("error_command_role_permissions")
        await ctx.reply(content=msg)
    return facts
...
async def is_voice_only(ctx: Context, send_message: bool = True) -> bool:
    ...
    facts = jukebox.is_in_voice_channel(member=ctx.author) or await is_admin(ctx, send_message=False)
    if not facts and send_message:
